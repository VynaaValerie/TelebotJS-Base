module.exports = ({ bot, config, axios }) => {
  bot.command("merdeka", async (ctx) => {
    try {
      const res = await axios.get(`${config.vtechApiUrl}/api/berita/merdeka?apikey=${config.vtechApiKey}`, { timeout: 15000 });
      const items = res.data?.data?.slice(0, 5);
      if (!items?.length) return ctx.reply("❌ Berita tidak tersedia saat ini.");
      await ctx.reply(formatBerita(items, "Merdeka"), { parse_mode: "MarkdownV2", disable_web_page_preview: true });
    } catch (e) {
      await ctx.reply(`❌ Gagal ambil berita Merdeka\\. Coba lagi nanti\\.`, { parse_mode: "MarkdownV2" });
    }
  });
};
