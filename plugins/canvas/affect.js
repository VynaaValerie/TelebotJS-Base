...
module.exports = ({ bot, config, axios, InputFile }) => {
  bot.command("affect", async (ctx) => {
    ...
    try {
      ...
      const { data } = await axios.get(`${config.vtechApiUrl}/api/canvas/affect`, {
        params: { apikey: config.vtechApiKey, image }, responseType: "arraybuffer", timeout: 30000,
      });
      await ctx.api.deleteMessage(ctx.chat.id, wait.message_id).catch(() => {});
      return ctx.replyWithPhoto(new InputFile(Buffer.from(data), "affect.jpg"), { caption: "🖼 *Affect*", parse_mode: "Markdown" });
    } catch (err) {
      await ctx.api.deleteMessage(ctx.chat.id, wait.message_id).catch(() => {});
