...
module.exports = ({ bot, config, axios, InputFile }) => {
  bot.command("facepalm", async (ctx) => {
    ...
    try {
      ...
      const { data } = await axios.get(`${config.vtechApiUrl}/api/canvas/facepalm`, {
        params: { apikey: config.vtechApiKey, image }, responseType: "arraybuffer", timeout: 30000,
      });
      await ctx.api.deleteMessage(ctx.chat.id, wait.message_id).catch(() => {});
      return ctx.replyWithPhoto(new InputFile(Buffer.from(data), "facepalm.jpg"), { caption: "🤦 *Facepalm*", parse_mode: "Markdown" });
    } catch (err) {
      await ctx.api.deleteMessage(ctx.chat.id, wait.message_id).catch(() => {});
