module.exports = ({ bot, config, axios }) => {
  bot.command("gptoss", async (ctx) => {
    const input = ctx.message.text.split(" ").slice(1).join(" ").trim();
    if (!input) return ctx.reply("Kirim pesanmu: `/gptoss <teks>`", { parse_mode: "Markdown" });

    const thinking = await ctx.reply("💬 _GPT-OSS memproses..._", { parse_mode: "Markdown" });
    try {
      const { data } = await axios.get(`${config.vtechApiUrl}/api/ai/gptoss120b`, {
