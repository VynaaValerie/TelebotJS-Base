module.exports = ({ bot, config, axios }) => {
  bot.command("phi2", async (ctx) => {
    const input = ctx.message.text.split(" ").slice(1).join(" ").trim();
    if (!input) return ctx.reply("Kirim pesanmu: `/phi2 <teks>`", { parse_mode: "Markdown" });

    const thinking = await ctx.reply("🔬 _Phi-2 memproses..._", { parse_mode: "Markdown" });
    try {
      const { data } = await axios.get(`${config.vtechApiUrl}/api/ai/phi2`, {
