...
module.exports = ({ bot, config, axios }) => {
  bot.command("tebakemoji", async (ctx) => {
    const wait = await ctx.reply("😀 _Memuat soal..._", { parse_mode: "Markdown" });
    try {
      const { data } = await axios.get(`${config.vtechApiUrl}/api/game/tebakemoji`, {
      ...
      ctx.reply(`😀 *Tebak Emoji!*\n\n${q.soal || q.emoji || q.question}\n\n_60 detik untuk menjawab!_`, { parse_mode: "Markdown" });
    } catch (err) {
      await ctx.api.deleteMessage(ctx.chat.id, wait.message_id).catch(() => {});
