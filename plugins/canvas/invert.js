...
module.exports = ({ bot, config, axios, InputFile }) => {
  bot.command("invert", async (ctx) => {
    ...
    try {
      ...
      const { data } = await axios.get(`${config.vtechApiUrl}/api/canvas/invert`, {
        params: { apikey: config.vtechApiKey, image }, responseType: "arraybuffer", timeout: 30000,
      });
      await ctx.api.deleteMessage(ctx.chat.id, wait.message_id).catch(() => {});
      return ctx.replyWithPhoto(new InputFile(Buffer.from(data), "invert.jpg"), { caption: "🔄 *Invert*", parse_mode: "Markdown" });
    } catch (err) {
      await ctx.api.deleteMessage(ctx.chat.id, wait.message_id).catch(() => {});
