module.exports = ({ bot, config, axios }) => {
  bot.command("gemma12b", async (ctx) => {
    const input = ctx.message.text.split(" ").slice(1).join(" ").trim();
    if (!input) return ctx.reply("Kirim pesanmu: `/gemma12b <teks>`", { parse_mode: "Markdown" });

    const thinking = await ctx.reply("✨ _Gemma 12B memproses..._", { parse_mode: "Markdown" });
    try {
      const { data } = await axios.get(`${config.vtechApiUrl}/api/ai/gemma-3-12b`, {
