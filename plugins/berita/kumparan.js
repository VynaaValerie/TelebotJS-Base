module.exports = ({ bot, config, axios }) => {
  bot.command("kumparan", async (ctx) => {
    try {
      const res = await axios.get(`${config.vtechApiUrl}/api/berita/kumparan?apikey=${config.vtechApiKey}`, { timeout: 15000 });
      const items = res.data?.data?.slice(0, 5);
      if (!items?.length) return ctx.reply("❌ Berita tidak tersedia saat ini.");
      await ctx.reply(formatBerita(items, "Kumparan"), { parse_mode: "MarkdownV2", disable_web_page_preview: true });
    } catch (e) {
      await ctx.reply(`❌ Gagal ambil berita Kumparan\\. Coba lagi nanti\\.`, { parse_mode: "MarkdownV2" });
    }
  });
};
