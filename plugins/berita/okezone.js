module.exports = ({ bot, config, axios }) => {
  bot.command("okezone", async (ctx) => {
    try {
      const res = await axios.get(`${config.vtechApiUrl}/api/berita/okezone?apikey=${config.vtechApiKey}`, { timeout: 15000 });
      const items = res.data?.data?.slice(0, 5);
      if (!items?.length) return ctx.reply("❌ Berita tidak tersedia saat ini.");
      await ctx.reply(formatBerita(items, "Okezone"), { parse_mode: "MarkdownV2", disable_web_page_preview: true });
    } catch (e) {
      await ctx.reply(`❌ Gagal ambil berita Okezone\\. Coba lagi nanti\\.`, { parse_mode: "MarkdownV2" });
    }
  });
};
